...
Recall that to each reduced root system $\Phi$ and a commutative ring $R$ one can associate a split simple group $\GG(\Phi, R)$ called the \emph{Chevalley group}.
...
This group shares many properties with its stable counterpart $\mathrm{SK}_1(R) = \lim\limits_{l\to\infty}\Ko(\rA_\ell, R)$ which, in turn, is an essential direct summand of $\Ko(R)$.
...
A $B$-module $M$ is called \emph{uniquely $r$-divisible} if for every $m\in M$ there exists a sole $m'\in M$ such that $rm' = m$ (we denote such $m'$ by $\frac{m}{a}$).
...
\begin{tm} \label{a3map} Let $B$ be a ring, $a\in B$ and let $I$ be an ideal $B$ that is uniquely $a$-divisible.
...
\end{df}
Tulenbaev shows that factors $x(u,\,v_ka)$, $1\leq k\leq N$ commute~\cite[Lemma~1.1\,e)]{Tul} and that for any other decomposition $v=\sum_{j=1}^Mv'_j$ one has
...
$$(z^tu)w=\sum_{i<j}c_{ij} u_{ij}, \text{ where } u_{ij}=u_je_i-u_ie_j,\text{ and }c_{ij}=w_iz_j-w_jz_i.$$
Each $c_{ij}u_{ij}$ is orthogonal to $u$ and, because of the assumption $n\geq4$, has at least two zero coordinates.
Thus, $X_{u,wb}(a)=\prod_{i<j}x(u,\,a c_{ij} u_{ij})$. 
It is enough to prove the assertion of $(\mathrm d)$ in the special case $g=x_{hk}(r)$.
If $h\neq i,j$ or $\{h,k\}=\{i,j\}$ the vector $\phi(g)^*ac_{ij}u_{ij}$ still has two zero coordinates.

Now consider the case $j=h$, $i\neq k$. 
By~\cite[3.12]{vdK} one has $$g\,x(u,\,ac_{ij}u_{ij})g\inv=x(\phi(g)u,\,ac_{ij}\,\phi(g)^*u_{ij}).$$
Set $u' = \phi(g)u$, since
$\phi(g)^*u_{ij} = t_{kj}(-r) \cdot u_{ij} = u_je_i - u_ie_j + ru_i e_k =u'_{ij}+r\,u'_{ki},$ 
we obtain from~\cite[3.11]{vdK} that
$$x(\phi(g)u,\,ac_{ij}\,\phi(g)^* u_{ij})=x\left(u',\,c_{ij}a\,u'_{ij}\right)\cdot x\left(u',\,rc_{ij}a\,u'_{ki}\right).$$
Decomposing in this fashion each factor $g\,x(u,\,ac_{ij}u_{ij})g^{-1}$ for which $\phi(g)^*ac_{ij}\, u_{ij}$ does not have two zero coordinates, 
...
Then one has $X_{u,b^4rv}(b)=Y_{b^4ru,v}(b).$
\end{lm}
\begin{proof}
Compute $g=[Y_{-arx,v}(b),\,X_{u,ay}(b)]$ in two different ways using Lemmas~\ref{xproperties}~and~\ref{yproperties}:
$$ g = X_{t(b^2rx,-v)u,\,t(b^2rx,-v)^*yb}(b)X_{u,-yb}(b) = X_{u,\,by+b^4rv}(b)\, X_{u,-by}(b) = X_{u,b^4rv}(b),$$
$$ g = Y_{-brx,v}(b) Y_{t(u,b^2y)arx,\,t(u,b^2y)^*v}(b)= Y_{-brx,v}(b)\, Y_{brx+b^4ru,\,v}(b)=Y_{b^4ru,v}(b).\qedhere$$
\end{proof}
...
\begin{equation}T(F(u, bv)) = X_{\tilde u,c/a^{4m}\,\tilde v}(a^{2m}) = Y_{c/a^{4m}\, \tilde u,\tilde v}(a^{2m}) = T(S(ub, v)). \label{eq:last}\end{equation}
Here $\tilde u, \tilde v \in B^n$ are liftings of $u$ and $v$ such that $\lambda_a(\tilde u)=a^m u$, $\lambda_a(\tilde v)=a^m v$ and $\tilde u^t \tilde v = 0$.
...
$$\lambda_a(\tilde{x})=a^mMe_3,\ \lambda_a(\tilde{y})=a^mM^*e_3\text{ and }\tilde{x}^t\tilde{y}=a^{2m},\ \tilde{x}^t\tilde{v} = 0,\ \tilde{u}^t\tilde{y} =0.$$

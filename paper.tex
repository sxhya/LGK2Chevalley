...
Now we turn to the main result of this paper, namely, we construct the map $$T\colon\St(n,\,R_a[X],\,XR_a[X])\rightarrow\St(n,\,R\ltimes XR_a[X]),\ \text{ for} n\geq4.$$ 
...
Clearly, $M$ is $a$-divisible if and only if the morphism of principal localization $\lambda_a\colon M \to M_a$ is an isomorphism.

\begin{tm} \label{a3map} Let $B$ be a ring, $a\in B$ and let $I$ be an ideal $B$ that is $a$-divisible.
Then there exists a map $T\colon\St(n,\,B_a,\,I)\rightarrow\St(n,\,B)$ which makes the following diagram commute.
$$\xymatrix{ \St^*(2n,\,B,\,I)\ar@<-0.0ex>[rr]^{\iota}\ar@<-0.0ex>[d]_{\lambda_{a}^*}&&\St(2n,\,B)\ar@<-0.0ex>[d]^{\lambda_{a}^*}\\
             \St^*(2n,\,B_a,\,I)\ar@<-0.0ex>[rr]_{\iota}\ar@{-->}[rru]_{T}&&\St(2n,\,B_a)}$$ \end{tm}

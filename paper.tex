...
Centrality of $\Kt$ is a corollary of the following variant of Quillen's local-global principle,
...
if and only if the images $\lambda_M^*(g)\in\St(\Phi,\,R_M[X])$ of $g$ under localisation maps $\lambda_M\colonR\to R_M$ are trivial for all maximal ideals $M\trianglelefteq R$. \end{tm}
For the proof of the claim that \cref{tm:lg-principle} follows from \cref{tm:centrality} we refer the reader to the proof of Theorem~1 in \cite{SCh} which works verbatim in our case.

The assertion of \cref{tm:lg-principle} was known previously in the following two situations:
\begin{itemize}
 \item in the case $\Phi=\rA_\ell$, $\ell\geq 4$ it was proved by Tulenbaev, the proof see~\cite[Theorem~2.1]{Tul}
 \item $\Phi=\rE_\ell$, $\ell=6,7,8$ and the second-named author
\end{itemize}
Thus, the special cases $\Phi=\rA_3$ and $\Phi=\rD_\ell$, $\ell\geq 4$ of \cref{tm:lg-principle} are new and have not been known before.
We also notice that a symplectic counterpart of~\cref{lg-principle} was demonstrated recently by the first-named author.

On the other hand, the special case 
includes as a special case both and~\cite[Theorem~2]{SCh} in which
the cases were studied, respectively.


As a yet another motivation for ??? we mention that 

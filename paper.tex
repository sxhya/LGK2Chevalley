...
For the proof of the claim that \cref{tm:lg-principle} follows from \cref{tm:centrality} we refer the reader to \cite{SCh} (cf. with the proof of Theorem~1 ibid.).
...
In \cref{sec:patching} we explain how \cref{tm:lg-principle} can be reduced to its own special case $\Phi=\rA_3$ by means of the patching technique of~\cite{SCh}.
Next, in \cref{sec:yap} we describe a new ``transpose-symmetric'' presentation for the relative linear Steinberg group. 
Finally, we use this presentation in~\cref{sec:lgp} where we prove \cref{tm:lg-principle} in the case $\Phi=\rA_3$.

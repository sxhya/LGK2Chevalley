...
By definition, the Steinberg group $\St(\Phi, R)$ is presented by generators $x_\alpha(\xi)$, which model the unipotents $t_\alpha(\xi)$, subjected to Steinberg relations (see~\cref{sec:prelim}). 
There is a natural map $\phi\colon\St(\Phi, R)\twoheadrightarrow \GG(\Phi, R)$ sending $x_\alpha(\xi)$ to $t_\alpha(\xi)$ whose image equals $\E(\Phi, R)$.
...
The unstable analogue of the ``universality'' part of this theorem was proved by M.~Stein and W.~van~der~Kallen for $\Phi$ of rank $\geq 5$ (or even $\geq 4$ for $\Phi$ of type $\rA_\ell, \rB_\ell, \rC_\ell$, see \cite{St1, KaS})
However, until recently, the analogue of the ``centrality'' part was only known for $\Phi=\rA_\ell$, $\ell\geq 3$, see~\cite[Corollary~2]{vdK} and for other classical $\Phi$ ``in the stable range'',
i.\,e. when the dimension of $R$ is suffiently small in comparison with the rank of $\Phi$, see~\cite[Corollary~3.4]{St78}.
...
the group $\Kt(\rD_\ell, R)$ is a central subgroup of $\St(\rD_\ell, R)$. \end{tm}

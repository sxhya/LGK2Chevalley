...
\begin{equation} u = (w^t\cdot v)\cdot u = \sum_{p<q}u_{pq},\text{ where }u_{pq} = (u_pw_q - u_qw_p) \cdot (v_qe_p - v_pe_q)\in{}\!R^n, \label{eq:canonical}\end{equation}
...
\begin{df}[Tulenbaev] \label{df:TulX}
For $u \in R^n$ we denote by $D(u)$ the set consisting of all $v\in B^n$ such that $v=\sum_{k=1}^Nv_k$ for some $v_1,\ldots,v_N\in B^n$ and that $u^tv_k=0$ and $v_k$ has at least two zero coordinates for all $k$.
...
For $u$, $w\in B^n$, $v, v' \in D(u)$ such that $u^tw=0$, for $a$, $b\in I(u)$, $c\in B$, $g\in\St(n,\,B)$ the following facts are true.
\begin{enumerate}
...
Indeed, take $z\in B^n$ such that $z^tu=b$ and write the canonical decomposition, cf. \eqref{eq:canonical}:
...
Decomposing in this fashion each factor $g\,x(u,\,ac_{ij}u_{ij})g$ for which $\phi(g)^*ac_{ij}\, u_{ij}$ does not have two zero coordinates, 
...
Similarly to \cref{df:TulX} this definition is correct and does not depend neither on the order of factors nor on the choice of decomposition for $u$.
\end{df}
...
\begin{lm}
\label{x=y}
Let $w$, $u$, $z$, $v$, $x$, $y\in B^n$ be vectors and $a$, $r\in B$ be scalars such that $w^tu=a$, $z^tv=a$, $x^ty=a$.
Suppose that the pairs $(w,\,u)$, $(z,\,v)$ and $(x,\,y)$ are mutually orthogonal. Then $$ X_{u,v}(ra^3)=Y_{u,v}(ra^3).$$
\end{lm}
\begin{proof}
\begin{multline*}
 X_{u,v}(ra^3) = X_{u,y+vra^2}(a)\, X_{u,-y}(a) = X_{t(x,-vra)u,t(x,-vra)^*y}(a)X_{u,-y}(a) = [Y_{x,-v}(ra),\,X_{u,y}(a)] = \\
 = Y_{x,-v}(ra)\, Y_{t(u,ya)x,t(u,ya)^*v}(ra)= Y_{-x,v}(ra)\, Y_{x+ua^2,v}(ra)=Y_{u,v}(ra^3).\qedhere
\end{multline*}
\end{proof}

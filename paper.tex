...
The unstable analogue of the ``universality'' part of this theorem was obtained by M.~Stein and W.~van~der~Kallen in 1970's, see \cite{St1, KaS}.
However, until recently, the analogue of the ``centrality'' part of the theorem was only known for $\Phi=\rA_\ell$, $\ell\geq 4$, see~\cite[Corollary~2]{vdK} and for other classical $\Phi$ ``in the stable range'',
i.\,e. when the dimension of $R$ is suffiently small with respect to the rank of $\Phi$, see~\cite[Corollary~3.4]{St78}.
...
\cref{tm:centrality} follows from the following local-global principle.
We refer the reader to the proof ~\cite[Theorem~]{SCh}
